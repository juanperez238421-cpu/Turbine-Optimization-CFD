...
def _robust_state_matrix(df: pd.DataFrame, columns: list[str] | None = None) -> tuple[np.ndarray, list[str]]:
    if columns is None:
        preferred = [
            "phase_dx_original_px", "phase_dy_original_px", "phase_shift_magnitude_original_px",
            "normalized_interframe_mad", "pair_particle_like_blob_density_proxy_mean",
            "pair_optical_dark_region_fraction_proxy_mean", "pair_optical_dark_region_cx_proxy_mean",
            "pair_optical_dark_region_cy_proxy_mean", "coarse_disp_median_px", "coarse_disp_p90_px",
        ...
        if not columns:
            for c in df.select_dtypes(include=[np.number]).columns:
                if c in _STATE_EXCLUDE: continue
                x = pd.to_numeric(df[c], errors="coerce").to_numpy(float)
                if np.sum(np.isfinite(x)) >= max(3, int(0.5 * len(df))) and np.nanstd(x) > 1e-12:
                    columns.append(c)
                if len(columns) >= 8: break
    if not columns:
        X = np.arange(len(df), dtype=float)[:, None]
        return RobustScaler().fit_transform(X), ["row_index_fallback"]
    X = df[columns].to_numpy(float)
    for j in range(X.shape[1]):
        x = X[:, j]; med = np.nanmedian(x)
        if not np.isfinite(med): med = 0.0
        x[~np.isfinite(x)] = med; X[:, j] = x
    return RobustScaler(quantile_range=(10, 90)).fit_transform(X), columns


def _rank01(x: np.ndarray, maximize: bool = True) -> np.ndarray:
    x = np.asarray(x, dtype=float); finite = np.isfinite(x)
    out = np.full(len(x), 0.5, dtype=float)
    if np.sum(finite) < 2: return out
    vals = x[finite]; ranks = rankdata(vals, method="average")
    p = (ranks - 1.0) / max(len(vals) - 1, 1)
    if not maximize: p = 1.0 - p
    out[finite] = p
    return out


def _quality_proxy(df: pd.DataFrame) -> tuple[np.ndarray, list[str]]:
    """Selection-side quality proxy. PIVlab direct_fraction is deliberately excluded."""
    maximize = ["coarse_pce_p10", "coarse_ppr_p10", "coarse_pce_median", "coarse_ppr_median", "phase_response", "normalized_frame_correlation"]
    minimize = ["pair_saturated_fraction_ge250_max", "pair_large_bright_component_area_fraction_proxy_max"]
    parts, used = [], []
    for c in maximize:
    ...
    return (np.median(np.column_stack(parts), axis=1), used) if parts else (np.full(len(df), 0.5), [])
...
def select_uniform_baseline(df: pd.DataFrame, n: int) -> AdvancedSelectionResult:
    n = _safe_n(df, n); pos = np.unique(np.round(np.linspace(0, len(df)-1, n)).astype(int))
    if len(pos) < n:
        remaining = [i for i in range(len(df)) if i not in set(pos)]; pos = np.r_[pos, remaining[:n-len(pos)]]
...
def select_stratified_random_baseline(df: pd.DataFrame, n: int, seed: int = 20260912) -> AdvancedSelectionResult:
    n = _safe_n(df, n); rng = np.random.default_rng(seed); edges = np.linspace(0, len(df), n + 1).astype(int); chosen = []
    for a, b in zip(edges[:-1], edges[1:]):
        if b > a: chosen.append(int(rng.integers(a, b)))
...
def select_random_iid(df: pd.DataFrame, n: int, seed: int = 20260912) -> AdvancedSelectionResult:
    n = _safe_n(df, n); rng = np.random.default_rng(seed); pos = rng.choice(len(df), size=n, replace=False)
...
def select_temporal_quality_stratified(df: pd.DataFrame, n: int) -> AdvancedSelectionResult:
    n = _safe_n(df, n); q, used = _quality_proxy(df); edges = np.linspace(0, len(df), n + 1).astype(int); chosen = []
    for a, b in zip(edges[:-1], edges[1:]):
        if b <= a: continue
        local = np.arange(a, b); chosen.append(int(local[np.argmax(q[local])]))
...
def select_kennard_stone(df: pd.DataFrame, n: int) -> AdvancedSelectionResult:
    n = _safe_n(df, n); X, cols = _robust_state_matrix(df)
    if n >= len(df): return AdvancedSelectionResult("kennard_stone", n, df["pair_index"].to_numpy(int), cols)
    D = cdist(X, X); i, j = np.unravel_index(np.argmax(D), D.shape); selected = [int(i)]
    if n > 1 and j != i: selected.append(int(j))
    while len(selected) < n:
        dmin = np.min(D[:, selected], axis=1); dmin[selected] = -np.inf; selected.append(int(np.argmax(dmin)))
...
def select_kernel_herding(df: pd.DataFrame, n: int) -> AdvancedSelectionResult:
    n = _safe_n(df, n); X, cols = _robust_state_matrix(df); D = cdist(X, X)
    positive = D[D > 0]; sigma = float(np.median(positive)) if len(positive) else 1.0
    K = np.exp(-0.5 * (D / max(sigma, 1e-12)) ** 2); target = K.mean(axis=0)
    selected = []; sum_kernel = np.zeros(len(df)); available = np.ones(len(df), bool)
    for t in range(1, n + 1):
        redundancy = sum_kernel / max(t - 1, 1); score = target - redundancy; score[~available] = -np.inf
        j = int(np.argmax(score)); selected.append(j); sum_kernel += K[j]; available[j] = False
...
def select_d_optimal(df: pd.DataFrame, n: int, ridge: float = 1e-6) -> AdvancedSelectionResult:
    n = _safe_n(df, n); X, cols = _robust_state_matrix(df); X = np.column_stack([np.ones(len(X)), X]); p = X.shape[1]
    Ainv = np.eye(p) / max(ridge, 1e-12); selected = []; available = np.ones(len(df), bool)
    for _ in range(n):
        gains = np.einsum("ij,jk,ik->i", X, Ainv, X); gains[~available] = -np.inf; j = int(np.argmax(gains)); x = X[j:j+1].T
        denom = float(1.0 + (x.T @ Ainv @ x)[0, 0]); Ainv = Ainv - (Ainv @ x @ x.T @ Ainv) / max(denom, 1e-12)
        selected.append(j); available[j] = False
...
def select_mmr_maximin(df: pd.DataFrame, n: int, min_gap_pairs: int = 1) -> AdvancedSelectionResult:
    n = _safe_n(df, n); X, cols = _robust_state_matrix(df); q, used = _quality_proxy(df); pair_ids = df["pair_index"].to_numpy(int)
    center = np.median(X, axis=0); selected = []; available = np.ones(len(df), bool)
    for _ in range(n):
        if not selected:
            state_novelty = np.linalg.norm(X-center[None,:], axis=1); temporal_novelty = np.abs(pair_ids-np.median(pair_ids))
        else:
            state_novelty = np.min(cdist(X, X[selected]), axis=1)
            temporal_novelty = np.array([min(abs(pid-pair_ids[s]) for s in selected) for pid in pair_ids], float)
        ...
        if selected and min_gap_pairs > 1:
            ...
            if np.any(available & ~bad): compromise[bad] = -np.inf
        j = int(np.argmax(compromise)); selected.append(j); available[j] = False
...
def _validate_reliability(df: pd.DataFrame, reliability: np.ndarray) -> np.ndarray:
    ...
    if R.ndim != 2 or R.shape[0] != len(df): raise ValueError("reliability must have shape [n_pairs, n_spatial_cells]")
...
def select_spatial_saturated_coverage(df: pd.DataFrame, n: int, reliability: np.ndarray, target_fraction_of_n: float = 0.25) -> AdvancedSelectionResult:
    n = _safe_n(df, n); R = _validate_reliability(df, reliability); target = max(1.0, float(n)*float(target_fraction_of_n))
    coverage = np.zeros(R.shape[1]); selected = []; available = np.ones(len(df), bool)
    for _ in range(n):
        current = np.minimum(coverage, target).sum(); gains = np.minimum(coverage[None,:] + R, target).sum(axis=1) - current; gains[~available] = -np.inf
        j = int(np.argmax(gains)); selected.append(j); coverage += R[j]; available[j] = False
...
def select_spatial_lower_tail(df: pd.DataFrame, n: int, reliability: np.ndarray, lower_tail_fraction: float = 0.10) -> AdvancedSelectionResult:
    n = _safe_n(df, n); R = _validate_reliability(df, reliability); coverage = np.zeros(R.shape[1]); selected = []; available = np.ones(len(df), bool)
    k = max(1, int(np.ceil(R.shape[1]*lower_tail_fraction)))
    for _ in range(n):
        cand_cov = coverage[None,:] + R; part = np.partition(cand_cov, kth=k-1, axis=1)[:, :k]; tail_mean = part.mean(axis=1); logcov = np.log1p(cand_cov).mean(axis=1)
        score = np.minimum(_rank01(tail_mean, True), _rank01(logcov, True)); score[~available] = -np.inf
        j = int(np.argmax(score)); selected.append(j); coverage += R[j]; available[j] = False
...
def select_quality_facility_maximin(df: pd.DataFrame, n: int, min_gap_pairs: int = 1) -> AdvancedSelectionResult:
    n = _safe_n(df, n); X, cols = _robust_state_matrix(df); q, used = _quality_proxy(df); D = cdist(X, X)
    positive = D[D > 0]; sigma = float(np.median(positive)) if len(positive) else 1.0; S = np.exp(-(D/max(sigma, 1e-12))**2)
    best = np.zeros(len(df)); pair_ids = df["pair_index"].to_numpy(int); selected = []; available = np.ones(len(df), bool)
    for _ in range(n):
        facility_gain = np.sum(np.maximum(best[:,None], S)-best[:,None], axis=0)
        temporal = np.array([min(abs(pid-pair_ids[s]) for s in selected) for pid in pair_ids], float) if selected else np.abs(pair_ids-np.median(pair_ids))
        score = np.minimum.reduce([_rank01(q, True), _rank01(facility_gain, True), _rank01(temporal, True)]); score[~available] = -np.inf
        if selected and min_gap_pairs > 1:
            bad = np.array([any(abs(pid-pair_ids[s]) < min_gap_pairs for s in selected) for pid in pair_ids])
            if np.any(available & ~bad): score[bad] = -np.inf
        j = int(np.argmax(score)); selected.append(j); best = np.maximum(best, S[:,j]); available[j] = False
