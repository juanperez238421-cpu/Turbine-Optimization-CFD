...
def _load_one(path: Path):
    ...
    if not m: raise ValueError(f"{path}: metadata header could not be parsed")
...
def load_pivlab_oracle(paths:list[str|Path])->OracleData:
    """
    Load only retrospective vector-validity evidence for selector QA.

    Velocity statistics are deliberately NOT copied into `pool`: otherwise representative
    selectors could use downstream PIV velocity fields to choose pairs, which would leak
    oracle information into a test intended to evaluate selection logic.
    """
    paths=[Path(p) for p in paths]
    if not paths: raise ValueError("No PIVlab files supplied")
    rows=[]; direct_rows=[]; files=[]; coords_ref=None; active_ref=None
    for path in sorted(paths):
        meta,df=_load_one(path); coords=df[["x [m]","y [m]"]].to_numpy(float); vt=df["Vector type [-]"].astype(int).to_numpy(); active=vt!=0
        if coords_ref is None: coords_ref=coords; active_ref=active
        else:
        ...
        rows.append({
            "pair_index":meta["frame_a"],"frame_a":meta["frame_a"],"frame_b":meta["frame_b"],"export_frame":meta["export_frame"],
            "xy_m_per_px":meta["xy_m_per_px"],"uv_mps_per_px_per_frame":meta["uv_mps_per_px_per_frame"],
    ...
    return OracleData(pool=pd.DataFrame(rows).iloc[order].reset_index(drop=True),direct_reliability=np.vstack(direct_rows)[order],coordinates_m=coords_ref[active_ref],active_mask_reference=active_ref,files=files)
...
def coverage_metrics(Rsel:np.ndarray)->dict:
    ...
    return {"coverage_mean":float(np.mean(c)),"coverage_min":float(np.min(c)),"coverage_q05":float(np.quantile(c,0.05)),"coverage_q10":float(np.quantile(c,0.10)),"coverage_median":float(np.median(c)),"fraction_cells_observed_at_least_once":float(np.mean(c>=1)),"fraction_cells_observed_at_least_twice":float(np.mean(c>=2)),"log_coverage_utility":float(np.mean(np.log1p(c)))}
...
def exact_best_subset(R:np.ndarray,n:int,objective:str="log")->tuple[tuple[int,...],float]:
    ...
    best_combo=None; best_value=-np.inf
...
def save_oracle_inventory(data:OracleData,outdir:str|Path)->None:
    outdir=Path(outdir); outdir.mkdir(parents=True,exist_ok=True); data.pool.to_csv(outdir/"oracle_pool.csv",index=False); np.save(outdir/"oracle_direct_reliability.npy",data.direct_reliability)
    pd.DataFrame(data.coordinates_m,columns=["x_m","y_m"]).to_csv(outdir/"oracle_active_coordinates.csv",index=False)
    summary={"n_fields":len(data.pool),"n_active_cells":int(data.direct_reliability.shape[1]),"pair_indices":data.pool["pair_index"].astype(int).tolist(),"direct_fraction_min":float(data.pool["direct_fraction"].min()),"direct_fraction_max":float(data.pool["direct_fraction"].max()),"direct_fraction_mean":float(data.pool["direct_fraction"].mean()),"files":data.files,"warning":"Retrospective validity oracle only. Velocity fields are intentionally excluded from selector inputs; direct-vector validity must not leak into prospective selection."}
